...
There are 2 main types of MCP Server - **local** which you download and run on your machine and **remote** that you connect to through a URL (just like an API).  They can both be configured with properties and they both have their risks - so we need to consider how we safely manage these in our environment.
...
- **Isolate** downloaded tools to limit the risk of command injection, for example running the MCP server within a Docker container.
