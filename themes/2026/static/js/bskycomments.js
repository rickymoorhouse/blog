...
/**
 * Fisher-Yates shuffle: return a new array with elements in random order.
 */
...
function renderHeader(stats, repostedBy, { engageText, postUrl: url, headerTemplate }) {
  if (headerTemplate) {
    ...
    if (engageText && url) {
      items.push(`<li class="engage"><a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(engageText)}</a></li>`)
    }
  ...
  if (stats.quoteCount > 0) {
    items.push(
      `<li class="quotes"><a href="${escapeHtml(url)}/quotes" target="_blank" rel="noopener noreferrer">${intword(stats.quoteCount)} ${pluralize(stats.quoteCount, 'quote,quotes')}</a></li>`
    )
  }
...
/**
 * Escape HTML special characters for safe interpolation into
 * double-quoted attributes and element content.
 */
...
export class BskyConversation extends HTMLElement {
  async connectedCallback() {
    const url = this.getAttribute('uri')
    if (!url) {
      console.warn('bsky-conversation: missing required "uri" attribute')
      this.innerHTML = ''
      return
    }
    ...
    if (!atUri) {
      console.warn(`bsky-conversation: could not parse URI "${url}". Expected a bsky.app post URL.`)
      this.innerHTML = ''
      return
    }
    ...
    try {
      const fetchJson = async (url) => {
        try {
          ...
          if (!r.ok) {
            console.warn(`bsky-conversation: API returned ${r.status} for ${url}`)
            return {}
          }
          return await r.json()
        } catch (err) {
          console.warn('bsky-conversation: fetch failed', err.message)
          return {}
        }
      }
  ...
  render(originalUrl, threadRes, quotesRes, repostsRes, maxDepth) {
    ...
    if (!document.getElementById('bsky-conversation-styles')) {
      const style = document.createElement('style')
      style.id = 'bsky-conversation-styles'
      style.textContent = `
        ...
        .bsky-conversation a {
          color: var(--bsky-link-color, black);
          text-decoration: underline;
          text-decoration-color: var(--bsky-link-underline, rgba(82, 82, 91, 0.5));
          font-weight: 400;
          transition: color 150ms, text-decoration-color 150ms;
        }
        .bsky-conversation a:hover {
          color: var(--bsky-link-hover, #2563eb);
          text-decoration-color: var(--bsky-link-underline-hover, rgba(59, 130, 246, 0.3));
        }
        ...
        .bsky-conversation .handle {
          color: var(--bsky-muted-color, #6b7280);
        }
        ...
        .bsky-conversation .thread {
          ...
          padding: 0 0 0 1.5em;
          margin: 0.5em 0 0;
          border-left: 2px solid var(--bsky-border-color, #e5e7eb);
        }
    ...
    this.innerHTML = `
      <div class="bsky-conversation">
        ${header}
        ${timeline}
        ${timeline && engageText ? `<footer class="continue"><a href="${escapeHtml(originalUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(engageText)}</a></footer>` : ''}
      </div>`
  }
}
